...
export default function PlotPage() {
  ...
  const removeFileGpxParsed = (fileKeyToRemove: number): void => {
    setFilesGpxParsed((prev) =>
      prev.filter((file) => file.key !== fileKeyToRemove),
    );
  };
  ...
  const updateDataInChart = useCallback(() => {
    const { elevPoints, distPoints } = getDataPointsAxis(
      filesGpxParsed.map((file) => file.fileParsed),
    );
  ...
  function handleOrderChange(newOrder: GpxSummaryData[]) {
    ...
    console.log({
      old_order: filesGpxParsed,
      new_order: reorderedFilesGpxParsed,
    });
      ...
      >
        ...
        <div className="mt-4 flex flex-col items-center shadow-2xl rounded-lg border-2 border-gray-300">
          <div className="flex flex-wrap flex-row space-x-2 space-y-2">
            ...
            />
            <CheckBox
              onChange={setChartAxisVisual}
              checked={chartAxisVisual}
              label={"Show Axis"}
              title="Show or hide the chart axis"
            />
            <CheckBox
              onChange={setChartPointsVisual}
              checked={chartPointsVisual}
              label={"Show Points"}
              title="Show or hide the points on the chart"
            />
            <CheckBox
              onChange={setChartSmoothVisual}
              checked={chartSmoothVisual}
              label={"Smooth Chart"}
          ...
          <div className="mt-2 mr-2 ml-2 flex flex-row flex-wrap items-center space-x-4 rounded-lg p-2 bg-violet-700 text-primary-foreground shadow hover:bg-violet-700/90 hover:scale-105 transition-transform duration-200 ease-in-out">
            <label className="text-sm font-medium flex items-center space-x-2">
              ...
              <input
                type="color"
                onChange={(color) => setChartLineColor(color.target.value)}
            ...
            <label className="text-sm font-medium flex items-center space-x-2">
              ...
              <div className="flex items-center space-x-2">
                <input
                  type="number"
                  onChange={(width) =>
                    setChartLineBorderWidth(parseInt(width.target.value))
                  }
                  defaultValue={chartLineBorderWidth}
                  id="widthLineChart"
                  min="1"
                  max="35"
          ...
          <label className="m-2 mb-0 rounded-lg p-2 flex items-center justify-between space-x-2 bg-violet-700 text-primary-foreground shadow hover:bg-violet-700/90 hover:scale-105 transition-transform duration-200 ease-in-out">
            ...
            <input
              type="color"
              onChange={(color) => setChartBackgroundColor(color.target.value)}
              defaultValue={chartBackgroundColor}
          ...
          <label className="m-2 rounded-lg p-2 flex flex-wrap items-center justify-between space-x-2 bg-violet-700 text-primary-foreground shadow hover:bg-violet-700/90 hover:scale-105 transition-transform duration-200 ease-in-out">
            <div className="space-x-2">
              ...
              <input
                type="text"
                onChange={(event) => setChartTitleText(event.target.value)}
                defaultValue={chartTitleText}
            ...
            <input
              type="color"
              onChange={(e) => setColorChartTitle(e.target.value)}
              defaultValue={colorChartTitle}
            ...
            <div className="space-x-2">
              <input
                type="number"
                onChange={(e) =>
                  setChartTitleFontSize(parseInt(e.target.value))
                }
                defaultValue={chartTitleFontSize}
                min="2"
                max="70"
                className="p-1 text-sm text-black border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 cursor-pointer"
