...
import FileCardListProps, {
  FileCardListRef,
} from "@/app/components/FileCardList/index.types";
import { GpxSummaryData } from "@/lib/types/gpx";

const FileCardList = React.forwardRef<FileCardListRef, FileCardListProps>(
  ({ onOrderChange, onCardRemove }, ref) => {
    ...
    const removeFileCardData = (cardKeyToRemove: number): void => {
      setFileCardsData((prev) =>
        prev.filter((card) => card.key !== cardKeyToRemove),
      );
    };
            ...
            >
              {fileCardsData.map((card, index) => (
                <FileCard
                  ...
                  value={card}
                  index={index}
                  onClickRemove={handleRemoveCardClick}
                />
              ))}
