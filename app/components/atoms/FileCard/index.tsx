...
const FileCard: FC<FileCardProps> = ({ value, index, onClickRemove }) => {
  ...
  return (
    <Draggable draggableId={value.key.toString()} index={index}>
      {(provided) => (
        <div
          className="mt-4 mb-0 mr-4 ml-4 w-52 bg-white shadow-xl border-2 border-violet-300 p-3  relative overflow-hidden rounded-xl"
        ...
        >
          ...
          <p className="text-sm text-zinc-500">
            ...
            {value.totalDistance ? value.totalDistance.toFixed(2) : "--"} <br />
            Max Elevation:{" "}
            {value.maxElevation ? value.maxElevation.toFixed(2) : "--"} <br />
            Min Elevation:{" "}
            {value.minElevation ? value.minElevation.toFixed(2) : "--"} <br />
            Difference in Altitude:{" "}
            {value.diffAltitude ? value.diffAltitude.toFixed(2) : "--"} <br />
            Positive Elevation difference:{" "}
            {value.posElevationDiff
              ? value.posElevationDiff.toFixed(2)
              : "--"}{" "}
            <br />
            Negative Elevation difference:{" "}
            {value.negElevationDiff
              ? value.negElevationDiff.toFixed(2)
              : "--"}{" "}
            <br />
          </p>
          <Button
